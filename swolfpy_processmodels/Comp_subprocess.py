...
def shredding(input_flow, Op_param, lci, flow_init):
    ...
    Shred_diesel = (
        ...
        / 1000
        * Op_param["Mtgp"]["amount"]
        * Op_param["Mtgf"]["amount"]
    )
    lci.add(name=("Technosphere", "Equipment_Diesel"), flow=Shred_diesel)
...
def ac_comp(input_flow, common_data, process_data, input_data, assumed_comp, lci, flow_init):
    # Degradation
    C_loss = (
        input_flow.data["C_input"].values
        * process_data["C_loss"].values
        / 100
        * input_data.Deg_Param["acDegProp"]["amount"]
        / 100
    )
    N_loss = (
        input_flow.data["N_input"].values
    ...
    Biofilter_NH3_to_N2O = (N_loss_as_NH3 - Biofilter_NH3) * input_data.Deg_Param["preNH3toN2O"][
        "amount"
    ]
    Biofilter_NH3_to_NOx = (N_loss_as_NH3 - Biofilter_NH3) * input_data.Deg_Param["preNH3toNOx"][
        "amount"
    ]
    ...
    lci.add(
        ...
        flow=(Biofilter_N2O + Biofilter_NH3_to_N2O)
    ...
    # Resource use
    Loader_diesel = (
        input_data.Op_Param["Tdoh"]["amount"]
        * input_data.Loader["hpfel"]["amount"]
        * input_data.Loader["Mffel"]["amount"]
    )
    Windrow_turner_diesel = (
        input_data.Op_Param["Tact"]["amount"]
        * input_data.AC_Turning["Fta"]["amount"]
        * input_flow.data["mass"]
        / 1000
        * input_data.AC_Turning["Mwta"]["amount"]
        * input_data.AC_Turning["Mwfa"]["amount"]
    )
    lci.add(name=("Technosphere", "Equipment_Diesel"), flow=Windrow_turner_diesel + Loader_diesel)
...
def vacuum(input_flow, sep_eff, Op_param, lci, flow_init):
    ...
    product.data["moist_cont"] = (
        input_flow.data["moist_cont"].values - vacuumed.data["moist_cont"].values
    )
...
def curing(input_flow, common_data, process_data, input_data, assumed_comp, lci, flow_init):
    ...
    C_loss = (
        ...
        * process_data["C_loss"].values
        / 100
        * (100 - input_data.Deg_Param["acDegProp"]["amount"])
        / 100
    )
    N_loss = (
    ...
    # Resource use
    Curing_diesel = (
        input_data.Op_Param["Tcur"]["amount"]
        * input_data.Curing["Ftc"]["amount"]
        * input_data.AC_Turning["Mwta"]["amount"]
        * input_data.AC_Turning["Mwfa"]["amount"]
        * input_flow.data["mass"].values
        / 1000
    )
    lci.add(name=("Technosphere", "Equipment_Diesel"), flow=Curing_diesel)
