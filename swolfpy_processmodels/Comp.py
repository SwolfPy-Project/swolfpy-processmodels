...
from .Comp_subprocess import ac_comp, add_water, curing, mix, post_screen, screen, shredding, vacuum
...
class Comp(ProcessModel):
    ...
    def calc(self):
        ...
        self.mixed = mix(self.S1_unders, self.shred, self.flow_init)
        ...
        if self.mixed.moist_cont > self.InputData.Deg_Param["initMC"]["amount"]:
            ...
            self.water_added = (
                self.InputData.Deg_Param["initMC"]["amount"] * self.mixed.flow - self.mixed.water
            ) / (1 - self.InputData.Deg_Param["initMC"]["amount"])

        water_flow = self.water_added * self.mixed.data["sol_cont"].values / self.mixed.solid

        self.substrate_to_ac = add_water(
            self.mixed, water_flow, self.Material_Properties, self.process_data, self.flow_init
        )

        ### Active Composting
        self.substrate_to_ps = ac_comp(
            self.substrate_to_ac,
            self.CommonData,
            self.process_data,
            self.InputData,
            self.Assumed_Comp,
        ...
        ### Post screen
        self.substrate_to_vac, self.ps_res = post_screen(
            self.substrate_to_ps,
        ...
        ### Curing
        self.final_comp = curing(
            self.substrate_to_cu,
            self.CommonData,
            self.process_data,
            self.InputData,
            self.Assumed_Comp,
        ...
        """
        solids = (
            self.mixed.data["sol_cont"].values
            - self.ps_res.data["sol_cont"].values
            - self.vac_res.data["sol_cont"].values
        )

        self.final_comp.data["P_cont"] = (
            solids
            * self.Material_Properties["Phosphorus Content"].values
            / 100
            * self.process_data["Degrades"].values
        )

        self.final_comp.data["K_cont"] = (
            solids
            * self.Material_Properties["Potassium Content"].values
            / 100
            * self.process_data["Degrades"].values
        )
        ...
        ### office
        Office_elec = (
            self.InputData.Office["Mta"]["amount"] * self.InputData.Office["Mea"]["amount"] / 1000
        ) / self.InputData.Op_Param["Taod"]["amount"]

        self.LCI.add(name=("Technosphere", "Electricity_consumption"), flow=Office_elec)

        ### Transportation
        self.LCI.add(
            name="Medium-duty truck transportation to land application",
            flow=(
                self.final_comp.data["mass"].values * self.InputData.Land_app["distLand"]["amount"]
            ),
        )

        self.LCI.add(
            name="Medium-duty empty return",
            flow=(
                self.final_comp.data["mass"].values
                / 1000
    ...
    def report(self):
        ...
        for y in self.Index:
            ...
            report["Technosphere"][y][("Technosphere", "Electricity_production")] = lci_report[
                ("Technosphere", "Electricity_production")
            ][y]
            report["Technosphere"][y][("Technosphere", "Equipment_Diesel")] = lci_report[
                ("Technosphere", "Equipment_Diesel")
            ][y]
            report["Technosphere"][y][
                ("Technosphere", "Internal_Process_Transportation_Medium_Duty_Diesel_Truck")
            ] = lci_report["Medium-duty truck transportation to land application"][y]
            report["Technosphere"][y][
                ("Technosphere", "Empty_Return_Medium_Duty_Diesel_Truck")
            ] = lci_report["Medium-duty empty return"][y]
            report["Technosphere"][y][("Technosphere", "Nitrogen_Fertilizer")] = lci_report[
                ("Technosphere", "Nitrogen_Fertilizer")
            ][y]
            report["Technosphere"][y][("Technosphere", "Phosphorous_Fertilizer")] = lci_report[
                ("Technosphere", "Phosphorous_Fertilizer")
            ][y]
            report["Technosphere"][y][("Technosphere", "Potassium_Fertilizer")] = lci_report[
                ("Technosphere", "Potassium_Fertilizer")
            ][y]
            report["Technosphere"][y][("Technosphere", "Peat")] = lci_report[
                ("Technosphere", "Peat")
            ][y]
            report["Technosphere"][y][("Technosphere", "compost_to_LF")] = lci_report[
                ("Technosphere", "compost_to_LF")
            ][y]
            report["Technosphere"][y][
                ("Technosphere", "market_for_excavation_skid_steer_loader")
            ] = lci_report[("Technosphere", "market_for_excavation_skid_steer_loader")][y]
            ...
            report["Biosphere"][y][
                ("biosphere3", "e4e9febc-07c1-403d-8d3a-6707bb4d96e6")
            ] = lci_report["Direct Carbon Storage and Humus Formation"][
                y
            ]  # Carbon dioxide, from soil or biomass stock ('air',)
            report["Biosphere"][y][
                ("biosphere3", "eba59fd6-f37e-41dc-9ca3-c7ea22d602c7")
            ] = lci_report["Carbon dioxide, non-fossil"][
                y
            ]  # Carbon dioxide, non-fossil ('air',)
            report["Biosphere"][y][
                ("biosphere3", "349b29d1-3e58-4c66-98b9-9d1a076efd2e")
            ] = lci_report["Carbon dioxide, fossil"][
                y
            ]  # Carbon dioxide, fossil (air,)
            report["Biosphere"][y][
                ("biosphere3", "20185046-64bb-4c09-a8e7-e8a9e144ca98")
            ] = lci_report["Dinitrogen monoxide"][
                y
            ]  # Dinitrogen monoxide ('air',)
            report["Biosphere"][y][
                ("biosphere3", "da1157e2-7593-4dfd-80dd-a3449b37a4d8")
            ] = lci_report["Methane, non-fossil"][
                y
            ]  # Methane, non-fossil ('air',)
            report["Biosphere"][y][
                ("biosphere3", "c1b91234-6f24-417b-8309-46111d09c457")
            ] = lci_report["Nitrogen oxides"][
                y
            ]  # Nitrogen oxides ('air',)
            report["Biosphere"][y][
                ("biosphere3", "d3260d0e-8203-4cbb-a45a-6a13131a5108")
            ] = lci_report["VOCs emitted"][
                y
            ]  # NMVOC, non-methane volatile organic compounds, unspecified origin ('air',)
            report["Biosphere"][y][
                ("biosphere3", "b9291c72-4b1d-4275-8068-4c707dc3ce33")
            ] = lci_report["Nitrate (ground water)"][
                y
            ]  # Nitrate ('water', 'ground-')
            report["Biosphere"][y][
                ("biosphere3", "7ce56135-2ca5-4fba-ad52-d62a34bfeb35")
            ] = lci_report["Nitrate (surface water)"][
                y
            ]  # Nitrate ('water', 'surface water')
            report["Biosphere"][y][
                ("biosphere3", "736f52e8-9703-4076-8909-7ae80a7f8005")
            ] = lci_report["Ammonium, ion (ground water)"][
                y
            ]  #'Ammonium, ion' (kilogram, None, ('water', 'ground-'))
            report["Biosphere"][y][
                ("biosphere3", "13331e67-6006-48c4-bdb4-340c12010036")
            ] = lci_report["Ammonium, ion (surface water)"][
                y
