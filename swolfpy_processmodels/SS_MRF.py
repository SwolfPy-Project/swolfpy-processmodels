...
class SS_MRF(ProcessModel):
    ...
    def calc(self):
        ...
        self._MS1_rmnd, self._MS1_rmvd = Man_Sort1(
            self._DF_feed,
        ...
        ### Vacuum
        self._Vac_rmnd, self._Vac_rmvd = Vacuum(
            self._MS1_rmvd,
            self.process_data[["Vacuum", "Manual-Vacuum"]].values,
            self.InputData,
            self.LCI,
        )
        ...
        self._DS1_rmnd, self._DS1_rmvd = DS1(
            self._MS1_rmnd,
            self.process_data[["Disc Screen 1", "Manual-DS1"]].values,
            self.InputData,
            self.LCI,
        )
        ...
        self._DS2_rmnd, self._DS2_rmvd = DS2(
            self._DS1_rmnd,
            self.process_data[["Disc Screen 2", "Manual-DS2"]].values,
            self.InputData,
            self.LCI,
        )

        ### Manual Sort 2-DS2 (Negative)
        self._MS2_DS2_rmnd, self._MS2_DS2_rmvd = MS2_DS2(
            self._DS2_rmvd,
            self.process_data["Manual Sort 2-DS2 (Negative)"].values,
            self.InputData,
            self.LCI,
        )
        ...
        self._DS3_rmnd, self._DS3_rmvd = DS3(
            self._DS2_rmnd,
            self.process_data[["Disc Screen 3", "Manual-DS3"]].values,
            self.InputData,
            self.LCI,
        )

        ### Manual Sort 2-DS3 (Negative)
        self._MS2_DS3_rmnd, self._MS2_DS3_rmvd = MS2_DS3(
            self._DS3_rmvd,
            self.process_data["Manual Sort 2-DS3 (Negative)"].values,
            self.InputData,
            self.LCI,
        )
        ...
        self._Mixed_Paper, self._ONP, self._OFF, self._Fiber_Other = Mixed_paper_separation(
            self._MS2_DS3_rmnd + self._MS2_DS2_rmnd, self.InputData
        )
        ...
        self._Baler_1Way_Baled = Baler_1Way(
            self._DS1_rmvd, self._MS2_DS3_rmnd + self._MS2_DS2_rmnd, self.InputData, self.LCI
        )

        ### Glass Breaker Screen
        self._GBS_rmnd, self._GBS_rmvd = GBS(
            self._DS3_rmnd,
            self.process_data["Glass Breaker Screen"].values,
            self.InputData,
            self.LCI,
        )

        ### Air Knife
        self._AK_rmnd, self._AK_rmvd = AK(
            self._GBS_rmvd, self.process_data["Air Knife"].values, self.InputData, self.LCI
        )
        self.LCI_Waste.add("Other_Residual", self._AK_rmvd)

        ### Optical Glass
        self._OG_rmnd, self._OG_rmvd = OG(
            self._AK_rmnd, self.process_data["Optical Glass"].values, self.InputData, self.LCI
        )
        self.LCI_Waste.add("Other_Residual", self._OG_rmnd)

        ### Manual Sort 3-G (Negative)
        self._MS3_G_rmnd, self._MS3_G_rmvd = MS3_G(
            self._OG_rmvd,
            self.process_data["Manual Sort 3-G (Negative)"].values,
            self.InputData,
            self.LCI,
        )
        ...
        (
            ...
            self._Green_glass,
            self._Mixed_Glass,
        ) = Glass_type(self._MS3_G_rmnd, self.InputData)
        ...
        ### Optical PET
        self._OPET_rmnd, self._OPET_rmvd = OPET(
            self._GBS_rmnd,
            self.process_data[["Optical PET", "Manual-OSPET"]].values,
            self.InputData,
            self.LCI,
        )

        ### Manual Sort 4-PET (Negative)
        self._MS4_PET_rmnd, self._MS4_PET_rmvd = MS4_PET(
            self._OPET_rmvd,
            self.process_data["Manual Sort 4-PET (Negative)"].values,
            self.InputData,
            self.LCI,
        )
        ...
        ### Optical HDPE
        self._OHDPE_rmnd, self._OHDPE_rmvd = OHDPE(
            self._OPET_rmnd,
            self.process_data[["Optical HDPE", "Manual-OSHDPE"]].values,
            self.InputData,
            self.LCI,
        )

        ### Manual Sort 4-HDPE (Negative)
        self._MS4_HDPE_rmnd, self._MS4_HDPE_rmvd = MS4_HDPE(
            self._OHDPE_rmvd,
            self.process_data["Manual Sort 4-HDPE (Negative)"].values,
            self.InputData,
            self.LCI,
        )
        ...
        ### Magnet
        self._Magnet_rmnd, self._Magnet_rmvd = Magnet(
            self._OHDPE_rmnd,
            self.process_data[["Magnet", "Manual-Magnet"]].values,
            self.InputData,
            self.LCI,
        )

        ### Manual Sort 4-Fe (Negative)
        self._MS4_Fe_rmnd, self._MS4_Fe_rmvd = MS4_Fe(
            self._Magnet_rmvd,
            self.process_data["Manual Sort 4-Fe (Negative)"].values,
            self.InputData,
            self.LCI,
        )
        ...
        self._ECS_rmnd, self._ECS_rmvd = ECS(
            self._Magnet_rmnd,
            self.process_data[["Eddy Current Separator", "Manual-ECS"]].values,
            self.InputData,
            self.LCI,
        )

        ### Manual Sort 4-Al (Negative)
        self._MS4_Al_rmnd, self._MS4_Al_rmvd = MS4_Al(
            self._ECS_rmvd,
            self.process_data["Manual Sort 4-Al (Negative)"].values,
            self.InputData,
            self.LCI,
        )
        ...
        self._MS5_rmnd, self._MS5_rmvd = MS5(
            self._ECS_rmnd,
            self.process_data["Manual Sort 5 (Positive)"].values,
            self.InputData,
            self.LCI,
        )
        ...
        self._Baler_2Way_Baled = Baler_2Way(
            self._Recovered_Container + self._Vac_rmvd, self.InputData, self.LCI
        )
        ...
        self._Mass_toConveyor = (
            ...
            + (
                self._DS1_rmnd + self._DS2_rmvd + self._DS2_rmnd + self._DS3_rmvd
                if self.InputData.Rec_material["Non_OCC_Fiber"]["amount"] > 0
                else 0
            )
            + self._DS3_rmnd
            + (
                self._GBS_rmvd + self._AK_rmnd + self._OG_rmvd
                if self.InputData.Rec_material["Glass"]["amount"] > 0
                else 0
            )
            + (
                self._GBS_rmnd + self._OPET_rmvd
                if self.InputData.Rec_material["PET"]["amount"] > 0
                else 0
            )
            + (
                self._OPET_rmnd + self._OHDPE_rmvd
                if self.InputData.Rec_material["HDPE"]["amount"] > 0
                else 0
            )
            + (
                self._OHDPE_rmnd + self._Magnet_rmvd
                if self.InputData.Rec_material["Ferrous"]["amount"] > 0
                else 0
            )
            + (
                self._Magnet_rmnd + self._ECS_rmvd
                if self.InputData.Rec_material["Aluminous"]["amount"] > 0
                else 0
            )
            + self._ECS_rmnd
        )
    ...
    def report(self):
        ...
        self.SS_MRF["Waste"] = self.LCI_Waste.report(
            input_mass=self._Input, transpose=True
        ).to_dict()

        # Technosphere
        tech_index = [x for x in self.LCI.col_dict.keys() if "biosphere3" not in x]
        self.SS_MRF["Technosphere"] = (
