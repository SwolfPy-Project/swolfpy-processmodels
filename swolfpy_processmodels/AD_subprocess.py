...
def Reactor(input, CommonData, process_data, AD_input, Material_Properties, EmissionFactor_Engine, EmissionFactor_Flare, LCI, flow_init):
    ...
    # ENergy content
    CH4_Energy_EngCont = CH4_prod_vol * AD_input.Biogas_gen['ad_collEff']['amount'] * (1 - AD_input.Biogas_gen['ad_downTime']['amount'])\
        * AD_input.Biogas_gen['ad_ch4EngCont']['amount']
    CH4_Flare_EngCont = CH4_prod_vol * AD_input.Biogas_gen['ad_collEff']['amount'] * AD_input.Biogas_gen['ad_downTime']['amount']\
        * AD_input.Biogas_gen['ad_ch4EngCont']['amount']
...
def Dewater(input, CommonData, process_data, AD_input, Material_Properties, added_water, assumed_comp, LCI, flow_init):
    if AD_input.AD_operation['isDw']['amount'] == 1:
        ...
        liq_rem = (input.data['moist_cont'].values - AD_input.Dewater['ad_mcDigestate']['amount'] * input.data['mass'].values)\
            /(1 - AD_input.Dewater['ad_mcDigestate']['amount'])
...
def AD_curing(input, input_to_reactor, CommonData, process_data, AD_input, assumed_comp, Material_Properties, LCI, flow_init):
        if AD_input.AD_operation['isCured']['amount'] == 1:
            ...
            # Carbon balance
            C_Remain = np.where(input.data['C_cont'].values / input_to_reactor.data['C_cont'].apply(lambda x: 1 if x <= 0 else x ).values
                                <= (1 - process_data['Percent Carbon loss during curing'].values / 100),
                                input.data['C_cont'].values,
            ...
            add_LCI('Methane, non-fossil',
                    C_loss_as_CH4 * CommonData.MW['CH4']['amount'] / CommonData.MW['C']['amount'],
                    LCI)
            add_LCI('Carbon dioxide, non-fossil _ Curing',
                    C_loss_as_CO2 * CommonData.MW['CO2']['amount'] / CommonData.MW['C']['amount'],
                    LCI)
            ...
            add_LCI('Ammonia',
                    N_loss_as_NH3 * CommonData.MW['Ammonia']['amount'] / CommonData.MW['N']['amount'],
                    LCI)
            add_LCI('Dinitrogen monoxide',
                    N_loss_as_N2O * CommonData.MW['Nitrous_Oxide']['amount'] / CommonData.MW['N']['amount'] / 2,
                    LCI)
            ...
            add_LCI('NMVOC, non-methane volatile organic compounds, unspecified origin',
                    VOC_emitted,
                    LCI)
...
def POTW (liq_treatment_vol, liq_rem, input_to_reactor, Dig_to_Curing, FinalCompost, index,AD_input, assumed_comp, Material_Properties, CommonData, LCI):
    ...
    AF_total['Phosphate'] = sum((1 - Material_Properties['Moisture Content'].values / 100) * assumed_comp * FinalCompost.data['mass'].values
                                / AD_input.Material_Properties['ad_densFC']['amount'] * Material_Properties['Phosphorus Content'].values / 100 * 1000)
